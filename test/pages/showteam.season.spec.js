describe('Page.ShowteamSeason', () => {
	...
	it('should extract match days', (done) => {
		...
		Fixture.getDocument('showteam.php?s=6', doc => {
			...
			expect(data.team.matchDays[0].competition).toEqual(Competition.SUPERCUP);
	...
	it('should extract match days and extend page', (done) => {
		...
		Fixture.getDocument('showteam.php?s=6', doc => {
			...
			expect(doc.getElementsByTagName('table')[2].rows[23].cells[1].textContent).toContain('OSE (3. Gruppenspiel)');
